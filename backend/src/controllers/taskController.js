...
export const createTask = async (req, res, next) => {
  try {
    const { workspaceId } = req.params;
    const { title, description, status, priority, assignedTo, dueDate, type, area, tags, archived } = req.body;
...
export const updateTask = async (req, res, next) => {
  try {
    ...
    const { title, description, status, priority, dueDate, type, assignedTo, area, tags, archived } = req.body;
    ...
    if (area !== undefined) task.area = area;
    ...
    // Notify newly assigned users only (people who weren't already assigned)
    if (assigneesChanged) {
